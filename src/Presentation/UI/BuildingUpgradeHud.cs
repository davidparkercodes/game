...
public partial class BuildingUpgradeHud : Control
{
    ...
    private void UpdateCurrentStats()
    {
        ...
        // Update cost without lightning bolts for current stats
        if (canUpgrade)
        {
            int upgradeCost = _upgradeService.GetUpgradeCost(_selectedBuilding);
            _costValue.Text = $"[color=#4eadc7]{_selectedBuilding.Cost}[/color] [color=white]+[/color] [color=#ff66cc]{upgradeCost}[/color]";
        }
        else
        {
            _costValue.Text = $"[color=#4eadc7]{_selectedBuilding.Cost}[/color] [color=white](MAX)[/color]";
        }

        // Update stats with new color
        if (canUpgrade)
        {
            _damageValue.Text = $"[color=#4eadc7]{_selectedBuilding.Damage}[/color] [color=white]→[/color] [color=#ff66cc]{upgradeStats.damage}[/color]";
            _rangeValue.Text = $"[color=#4eadc7]{_selectedBuilding.Range:F0}[/color] [color=white]→[/color] [color=#ff66cc]{upgradeStats.range:F0}[/color]";
            _attackSpeedValue.Text = $"[color=#4eadc7]{_selectedBuilding.AttackSpeed:F0}[/color] [color=white]→[/color] [color=#ff66cc]{upgradeStats.attackSpeed:F0}[/color]";
        }
        else
        {
            // No arrows at max level
            _damageValue.Text = $"[color=#4eadc7]{_selectedBuilding.Damage}[/color] [color=white](MAX)[/color]";
            _rangeValue.Text = $"[color=#4eadc7]{_selectedBuilding.Range:F0}[/color] [color=white](MAX)[/color]";
            _attackSpeedValue.Text = $"[color=#4eadc7]{_selectedBuilding.AttackSpeed:F0}[/color] [color=white](MAX)[/color]";
        }
    }
