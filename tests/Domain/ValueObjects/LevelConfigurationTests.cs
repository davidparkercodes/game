...
namespace Game.Tests.Domain.ValueObjects
{
    ...
    {
        [Fact]
        public void Constructor_WithValidParameters_ShouldCreateLevelConfiguration()
        {
            // Arrange
            var pathPoints = new[]
            {
            ...
            var config = new LevelConfiguration(
                ...
                pathWidth: 64.0f,
                spawnPoint: new PathPoint(0, 0),
                endPoint: new PathPoint(200, 0),
                initialMoney: 150,
                initialLives: 25
            );
            ...
            string levelName, string description, float pathWidth, int initialMoney, int initialLives)
        {
            ...
            Assert.Throws<ArgumentException>(() => new LevelConfiguration(
                levelName, description, pathPoints, pathWidth,
                new PathPoint(0, 0), new PathPoint(100, 100),
                initialMoney, initialLives
            ));
        }
        ...
        {
            // Act
            var config = LevelConfiguration.CreateDefault();
        ...
        public void PathLength_ShouldCalculateCorrectly()
        {
            ...
            var config = new LevelConfiguration(
                "Test", "", pathPoints, 64.0f,
                pathPoints[0], pathPoints[^1], 100, 20
            );
        ...
        public void DifficultyRating_ShouldCalculateBasedOnComplexityAndResources()
        {
            ...
            var easyLevel = new LevelConfiguration(
                "Easy", "", simplePathPoints, 64.0f,
                simplePathPoints[0], simplePathPoints[1], 1000, 50 // Lots of money and lives
            );

            var hardLevel = new LevelConfiguration(
                "Hard", "", complexPathPoints, 64.0f,
                complexPathPoints[0], complexPathPoints[^1], 50, 5 // Little money and lives
            );
        ...
        public void Equals_WithIdenticalConfigurations_ShouldReturnTrue()
        {
            ...
            var config1 = new LevelConfiguration(
                "Test", "desc", pathPoints, 64.0f,
                pathPoints[0], pathPoints[1], 100, 20
            );

            var config2 = new LevelConfiguration(
                "Test", "desc", pathPoints, 64.0f,
                pathPoints[0], pathPoints[1], 100, 20
            );
        ...
        public void ToString_ShouldContainRelevantInformation()
        {
            ...
            var config = new LevelConfiguration(
                "Test Level", "", pathPoints, 64.0f,
                pathPoints[0], pathPoints[^1], 150, 25
            );
